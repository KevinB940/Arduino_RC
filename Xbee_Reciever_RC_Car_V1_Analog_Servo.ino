...
void loop() {
  ...
  if (Serial.available()) {
    ...
    uint8_t steering = atoi(strtok((char*)input.c_str(), ",")); // Read upto comma and convert string to int
    ...
    if (steering ^ forwardBackward == checkSum) { // Send output to servos if chechsum match
      if (steering >= 0 && steering <= 180 && forwardBackward >= ESC_MIN && forwardBackward <= ESC_MAX) {
        timer = millis();

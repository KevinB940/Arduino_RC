...
void loop() {
  ...
  if (millis() - timer > 10) { // Limit the serial output frequency to 10 ms
    ...
    if (PS3.PS3Connected && millis() - PS3.getLastMessageTime() < 100) {
      ...
      uint16_t steering = map(PS3.getAnalogHat(RightHatX), 0, 255, 2200, 800);
      ...
      if (PS3.getButtonClick(PS)) {
        sendCommand(ESC_MID, ESC_MID); // Center steering servo and stop ESC
        PS3.disconnect();
      }
    } else
      sendCommand(ESC_MID, ESC_MID); // Center steering servo and send stop ESC if data has not been received from the controller in the last 100 ms
  }
}

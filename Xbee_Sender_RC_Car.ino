...
void setup() {
  ...
  EEPROM.get(0, trim); // Get trim value from the EEPROM
...
void updateLeds() {
  PS3.setLedRaw(sensitivity * 16.0f); // Convert value from 0-1 to 0-16 and then just use the LED as a binary display
}
  ...
  if (millis() - timer > 10) { // Limit the serial output frequency to 10 ms
    ...
    if (PS3.PS3Connected && millis() - PS3.getLastMessageTime() < 100) {
      ...
      if (PS3.getButtonClick(RIGHT) && trim < 90) {
        trim++;
        EEPROM.put(0, trim); // Write value to EEPROM
      ...
      steering = constrain(steering + trim, 0, 180); // Apply trim value
